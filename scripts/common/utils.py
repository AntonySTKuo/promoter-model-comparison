...
def sci_label(value, show_set=None):
    if show_set is not None and value not in show_set:
        return ''
    s = str(int(value) if float(value).is_integer() else value).replace('-', '–')
    return f'$10^{{{s}}}$'
...
def sci_ticks(ax, axis, ticksize=7, pad=4):
    ...
    if axis == 'x':
        ...
        ax.set_xticks(majorTicks, [sci_label(i) for i in majorTicks])
        ax.tick_params(axis='x', which='major', direction='in', width=0.3, length=1.8, labelsize=ticksize, pad=pad)
        ...
        ax.set_yticks(majorTicks, [sci_label(i) for i in majorTicks])
        ax.tick_params(axis='y', which='major', direction='in', width=0.3, length=1.8, labelsize=ticksize, pad=pad)
